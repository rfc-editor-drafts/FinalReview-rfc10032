...
It also specifies their use as stream ciphers and message authentication codes (MACs).
...
The `Encrypt` function of AEGIS-128X resembles that of AEGIS-128L. Similarly, the `Encrypt` function of AEGIS-256X mirrors that of AEGIS-256. Both process `R`-bit input blocks per update.
...
The `Decrypt` function of AEGIS-128X resembles that of AEGIS-128L. Similarly, the `Decrypt` function of AEGIS-256X mirrors that of AEGIS-256. Both process `R`-bit input blocks per update.
...
The `DecPartial` function is similar to the AEGIS-128L `DecPartial` function but decrypts a nonempty final ciphertext block shorter than `R` bits.
...
The `DecPartial` function is similar to the AEGIS-256 `DecPartial` function but decrypts a nonempty final ciphertext block shorter than `R` bits.
...
When AEGIS is used for authenticated encryption, a `nonce` MUST only be used once for a given `key`, even with different `tag` lengths. Failure to do so would immediately reveal the bitwise difference between two messages.
